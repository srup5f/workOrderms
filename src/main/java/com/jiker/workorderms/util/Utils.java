    ...
    public  Map<String, Object> produceWorkOrder(List<WorkOrderPlan> workOrderPlanList) {
        ...
        for (int i = 0; i < workOrderPlanList.size(); i++) {
            ...
            for (int n = 0; n < num; n++) {
                ...
                workOrder.setStartTime((java.sql.Date) dateAdd(workOrderPlanList.get(i).getPlanStartTime(), dateType, n));
                workOrder.setEndTime((java.sql.Date) dateAdd(workOrderPlanList.get(i).getPlanEndTime(), dateType, n + 1));
